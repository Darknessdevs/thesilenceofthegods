...
let mobileMenuOpen = false; // Nueva variable para tracking del menú
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setupMobileMenuEvents(); // Nueva función para eventos del menú móvil
    logWelcomeMessage();
});
...
function setupMobileMenuEvents() {
    ...
    document.addEventListener('click', function(event) {
        ...
        if (mobileMenuOpen && !mobileMenu.contains(event.target) && !hamburger.contains(event.target)) {
            closeMobileMenu();
        }
    });
